...
public class STGExtractor {
    ...
    public STGExtractor(GlobalConfig config, Settings settings) {
        ...
        try{
            ...
            backstageApp = runBackstage(settings);  // 运行Backstage从被测应用中提取控件及函数等信息
    ...
    private void updateActivity(Activity activity,  st.cs.uni.saarland.de.entities.Activity backActivity, Map<Integer, Dialog> dialogMap, Map<Integer, Menu> menuMap ) {
        ...
        for (Integer layoutId : backActivity.getXmlLayouts()) {
            ...
            // System.out.println("<check point> uiElement: " + uiElement);
            if (uiElement != null) {
    ...
    /**
     * Use BACKSTAGE to process the app 使用Backstage分析被测应用
     */
    private Application runBackstage(Settings settings) {
        ...
        List<UiElement> distinctUiElements = uiElementObjectsForReachabilityAnalysis
                .stream().distinct().collect(Collectors.toList()); // 获取被测应用中的所有控件
        ...
        System.out.println("<check point> distinctUiElements: " + distinctUiElements.toString()); // 输出为列表，列表中每一元素由一组（method, elementId, globalId）构成, 其中method为每个控件各自的监听器方法，elementedId为每个控件的Id
        System.out.println("<check point> appResults.getActivityNames(): " + appResults.getActivityNames()); // 输出从APK文件中分析得到的Activity
        writeListToFile(distinctUiElements, "/home/lgy/AndroidTools/GoalExplorer-4.3/GoalExplorer-main/ExperimentSubject/widgetInfo.txt");
        
        return AppController.getInstance().getApp();
    }
    
    // 将分析得到的控件信息，按行存储到指定路径下的文件中
    public static void writeListToFile(List<UiElement> list, String filePath){
