...
def install_app(avd_serial, apk_path, config_apk_path="")
  ...
  if !config_apk_path.empty?
    ...
    cmd = "adb -s #{avd_serial} install-multiple -r #{apk_path} #{config_apk_path} "
  else
    #cmd = "adb -s #{avd_serial} install -g -r -t #{apk_path} "
    cmd = "adb -s #{avd_serial} install -r -t #{apk_path} "
  end
...
# construct fsm for open source app, given app's dir, emma-instrumented 实际运行逻辑
def construct_fsm(apk_path, avd_serial, stoat_port, stg_file,
  ...
  # start the stoat client for gui exploration 通过命令行的方式启动并运行动态测试
...
if !apk_path.eql?('') && File.exist?(apk_path)
  ...
  install_app(avd_serial, apk_path, config_apk_path) # 安装应用

  # actual logic of the script
  construct_fsm(apk_path, avd_serial, stoat_port, stg_file,
                max_exploration_time, max_event_number, event_delay, strategy) # 实际的探索逻辑

  # cleanup
  # uninstall_app(avd_serial, apk_path) # 卸载应用
  # cleanup(avd_serial) # 关闭模拟器
else
